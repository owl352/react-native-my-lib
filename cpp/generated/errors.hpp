...
class NativeErrors : public jsi::HostObject {
private:
...
protected:
  ...
  jsi::Value cpp_uniffi_internal_fn_func_ffi__arraybuffer_to_string(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_ffi_errors_uniffi_contract_version(jsi::Runtime &rt,
                                                    const jsi::Value &thisVal,
                                                    const jsi::Value *args,
                                                    size_t count);

public:
  NativeErrors(jsi::Runtime &rt,
               std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
  virtual ~NativeErrors();

  /**
   * The entry point into the crate.
   *
   * React Native must call `NativeErrors.registerModule(rt, callInvoker)`
   ...
   * be the case.
   *
   * Clients should call `NativeErrors.unregisterModule(rt)` after final use
   * where possible.
   */
